...
class Collector_Fusion(object):
    ...
    def eval_batch_collect(
        self, scores_tensor_1: torch.Tensor, scores_tensor_2: torch.Tensor, \
        valid_users_confidence_1, valid_users_confidence_2, interaction, positive_u: torch.Tensor, positive_i: torch.Tensor
    ):
        """ Collect the evaluation resource from batched eval data and batched model output.
            Args:
                scores_tensor (Torch.Tensor): the output tensor of model with the shape of `(N, )`
        ...
        if self.register.need('rec.topk'):
            ...
            _, topk_idx_1 = torch.topk(scores_tensor_1, max(self.topk), dim=-1)  # n_users x k
            _, topk_idx_2 = torch.topk(scores_tensor_2, max(self.topk), dim=-1) 
            
            import pdb; pdb.set_trace()

            topk_idx = []
            
            pos_matrix = torch.zeros_like(scores_tensor_1, dtype=torch.int)
            pos_matrix[positive_u, positive_i] = 1
            pos_len_list = pos_matrix.sum(dim=1, keepdim=True)
            pos_idx = torch.gather(pos_matrix, dim=1, index=topk_idx)
            result = torch.cat((pos_idx, pos_len_list), dim=1)
