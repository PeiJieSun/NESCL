...
def get_tensorboard(config, suffix=None):
...
def notify(title, content):
    token = 'a5fe3dfaf647485e8c790b4710608342'
    base_url = 'http://www.pushplus.plus/send?'
    template = 'txt'
    channel = 'wechat' # 微信公众号发送

    content = content.replace(" ", "+")
    construct_msg = '{}token={}&title={}&content={}&template={}'.format(base_url, token, title, content, template)
    
    myURL = urlopen(construct_msg)