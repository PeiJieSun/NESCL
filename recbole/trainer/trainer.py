...
from recbole.evaluator import Evaluator, Collector, Collector_Fusion
...
class AbstractTrainer(object):
    ...
    def __init__(self, config, model, suffix=None):
        self.config = config
        self.model = model
        self.suffix = suffix
...
class AbstractTrainer_Fusion(object):
    r"""Trainer Class is used to manage the training and evaluation processes of recommender system models.
    AbstractTrainer is an abstract class in which the fit() and evaluate() method should be implemented according
    to different training and evaluation strategies.
    """

    def __init__(self, config_1, config_2, model_1 ,model_2, suffix=None):
...
class Trainer(AbstractTrainer):
    ...
    def __init__(self, config, model, suffix=None):
        super(Trainer, self).__init__(config, model, suffix)

        self.logger = getLogger()
        self.tensorboard = get_tensorboard(config, suffix)
    ...
    def _train_epoch(self, train_data, epoch_idx, loss_func=None, show_progress=False, flag=False):
        ...
        for batch_idx, interaction in enumerate(iter_data):
            ...
            losses, mf_loss, ssl_loss = loss_func(interaction, flag, epoch_idx, batch_idx)

            mf_loss_list.append(mf_loss.item())
            ssl_loss_list.append(ssl_loss.item())
        ...
        self.logger.info('mf_loss:%.4f' % np.sum(mf_loss_list))
        self.logger.info('ssl_loss_list:%4f' % np.mean(ssl_loss_list))

        return total_loss

    def _train_epoch_origin(self, train_data, epoch_idx, loss_func=None, show_progress=False):
        r"""Train the model in an epoch
    ...
    def _train_epoch_bak(self, train_data, epoch_idx, loss_func=None, show_progress=False, flag=False, item_clusters=None):
        r"""Train the model in an epoch
        ...
        for batch_idx, interaction in enumerate(iter_data):
            ...
            #losses, mf_loss = loss_func(interaction, flag)
            #losses, mf_loss = loss_func(interaction, epoch_idx, batch_idx, item_clusters)
            losses, mf_loss, cluster_loss, cluster_loss_2, item_clusters = loss_func(interaction, epoch_idx, batch_idx, item_clusters)

            mf_loss_list.append(mf_loss.item())
            cluster_loss_list.append(cluster_loss.item())
            cluster_2_loss_list.append(cluster_loss_2.item())
        ...
        #import pdb; pdb.set_trace()
        self.logger.info('mf_loss:%.4f' % np.sum(mf_loss_list))
        self.logger.info('co_clustering_loss:%4f' % np.mean(cluster_loss_list))
        self.logger.info('co_clustering_2_loss:%4f' % np.mean(cluster_2_loss_list))

        return total_loss, item_clusters

    def _train_epoch_2(self, train_data, epoch_idx, loss_func=None, show_progress=False, flag=False, item_clusters=None):
        r"""Train the model in an epoch
        ...
        Returns:
            ...
            tuple which includes the sum of loss in each part.
        """
        if epoch_idx >= self.config['cluster_start_epoch']:
            self.model.train()
            loss_func = loss_func or self.model.calculate_loss_2
            total_loss = None
            iter_data = (
                tqdm(
                    train_data,
            ...
            for batch_idx, interaction in enumerate(iter_data):
                ...
                #losses, mf_loss, co_clustering_loss = loss_func(interaction, flag)
                losses, mf_loss, cluster_loss, cluster_loss_2, item_clusters, node_xx_clusters = loss_func(interaction, epoch_idx, batch_idx, item_clusters)
            ...
            total_loss, item_clusters = None, None

        return item_clusters, node_xx_clusters
    ...
    def _save_checkpoint(self, epoch):
        ...
        #if torch.is_tensor(self.model.cluster_embedding):
        #    state['cluster_embedding'] = self.model.cluster_embedding
    ...
    def fit(self, train_data, valid_data=None, verbose=True, saved=True, show_progress=False, callback_fn=None, nni_mode=False):
        ...
        for epoch_idx in range(self.start_epoch, self.epochs):
            ...
            if (epoch_idx + 1) % self.eval_step == 0:
                ...
                if nni_mode:
                    nni.report_intermediate_result(valid_result[self.config['valid_metric'].lower()])
                ...
                #stop_flag = 0
                assert 'early_stop' in self.config

                if self.config['early_stop'] == False:
                    stop_flag = 0
                if stop_flag:
        ...
        #nni.report_final_result(self.best_valid_score)

        return self.best_valid_score, self.best_valid_result

    def fit_bak_2(self, train_data, valid_data=None, verbose=True, saved=True, show_progress=False, callback_fn=None, nni_mode=False):
        r"""Train the model based on the train data and the valid data.
        ...
        for epoch_idx in range(self.start_epoch, self.epochs):
            ...
            train_loss, item_clusters = self._train_epoch(train_data, epoch_idx, show_progress=show_progress, flag=flag_x, item_clusters=item_clusters)
    ...
    def fit_bak(self, train_data, valid_data=None, verbose=True, saved=True, show_progress=False, callback_fn=None, nni_mode=False):
        r"""Train the model based on the train data and the valid data.
        ...
        for epoch_idx in range(self.start_epoch, self.epochs):
            ...
            train_loss, _ = self._train_epoch(train_data, epoch_idx, show_progress=show_progress, flag=flag_x, item_clusters=item_clusters)
            ...
            #'''
            item_clusters, node_xx_clusters = self._train_epoch_2(train_data, epoch_idx, show_progress=show_progress, flag=flag_x, item_clusters=item_clusters)
            #train_loss, _ = self._train_epoch(train_data, epoch_idx, show_progress=show_progress, flag=flag_x, item_clusters=item_clusters)

            self.logger.info('clusters_num:')
            #import pdb; pdb.set_trace()
            self.logger.info(np.sum(node_xx_clusters.detach().cpu().numpy(), axis=0))
    ...
    @torch.no_grad()
    def evaluate(self, eval_data, load_best_model=True, model_file=None, show_progress=False, user_clicked_items=None, customize=False):
        r"""Evaluate the model based on the eval data.
        ...
        recommend_user_items_dict = {}
        stat_user_items_tensor = []
        ...
        for batch_idx, batched_data in enumerate(iter_data):
            ...
            if self.gpu_available and show_progress:
                iter_data.set_postfix_str(set_color('GPU RAM: ' + get_gpu_usage(self.device), 'yellow'))
            topk_idx, result = self.eval_collector.eval_batch_collect(scores, interaction, positive_u, positive_i)
            
            '''
            for idx, user in enumerate(interaction['user_id'].tolist()):
                user_token = eval_data.dataset.field2id_token['user_id'][user]
                item_list = ['user: %s'% user_token]
                clicked_item_set = set()
                for item in topk_idx.cpu().tolist()[idx]:
                    if item in user_clicked_items[user]:
                        item_token = eval_data.dataset.field2id_token['item_id'][item]
                        item_list.append('%s:%d, c' % (item_token, df[(int(user_token), int(item_token))]))
                        clicked_item_set.add(item)
                    else:
                        item_list.append('%s, n-c' % eval_data.dataset.field2id_token['item_id'][item])
                for item in user_clicked_items[user]:
                    if item not in clicked_item_set:
                        item_token = eval_data.dataset.field2id_token['item_id'][item]
                        item_list.append('%s:%d, n-r' % (item_token, df[(int(user_token), int(item_token))]))
                #import pdb; pdb.set_trace()
                recommend_user_items_dict[user] = item_list
                stat_user_items_tensor.append(result.tolist()[idx])
            #import pdb; pdb.set_trace()
            #'''
        ...
        if customize:
            result = self.evaluator.customize_evaluate('ndcg', struct)

            #stat_user_items_tensor = torch.LongTensor(stat_user_items_tensor)
            return result#, recommend_user_items_dict, torch.LongTensor(stat_user_items_tensor)
        else:
    ...
    @torch.no_grad()
    def evaluate_X(self, eval_data, load_best_model=True, model_file=None, show_progress=False, user_clicked_items=None):
        r"""Evaluate the model based on the eval data.
        ...
        for batch_idx, batched_data in enumerate(iter_data):
            ...
            if self.gpu_available and show_progress:
                iter_data.set_postfix_str(set_color('GPU RAM: ' + get_gpu_usage(self.device), 'yellow'))
            topk_idx, _ = self.eval_collector.eval_batch_collect(scores, interaction, positive_u, positive_i)
        
        self.eval_collector.model_collect(self.model)
        struct = self.eval_collector.get_data_struct()

        from recbole.evaluator.metrics import NDCGX
        ndcgx = NDCGX(self.config)
        ndcg_users = ndcgx.calculate_metric(struct)[:, self.config['topk'][0]-1]
...
class Trainer_Fusion(AbstractTrainer_Fusion):
    r"""The basic Trainer for basic training and evaluation strategies in recommender systems. This class defines common
    functions for training and evaluation processes of most recommender system models, including fit(), evaluate(),
    resume_checkpoint() and some other features helpful for model training and evaluation.
    ...
    def __init__(self, config_1, config_2, model_1, model_2, suffix=None):
        super(Trainer_Fusion, self).__init__(config_1, config_2, model_1, model_2, suffix)
        ...
        self.evaluator = Evaluator(config_1)
        self.item_tensor = None
        self.tot_item_num = None
    ...
    @torch.no_grad()
    def evaluate_X(self, eval_data, load_best_model=True, model_file_1=None, model_file_2=None, \
        valid_users_confidence_1=None, valid_users_confidence_2=None, show_progress=False, user_clicked_items=None):
        r"""Evaluate the model based on the eval data.
        ...
        if load_best_model:
            checkpoint_file_1 = model_file_1
            checkpoint_1 = torch.load(checkpoint_file_1)
            self.model_1.load_state_dict(checkpoint_1['state_dict'])
            self.model_1.load_other_parameter(checkpoint_1.get('other_parameter'))
            message_output = 'Loading model structure and parameters from {}'.format(checkpoint_file_1)
            self.logger.info(message_output)

            checkpoint_file_2 = model_file_2
            checkpoint_2 = torch.load(checkpoint_file_2)
            self.model_2.load_state_dict(checkpoint_2['state_dict'])
            self.model_2.load_other_parameter(checkpoint_2.get('other_parameter'))
            message_output = 'Loading model structure and parameters from {}'.format(checkpoint_file_2)
            self.logger.info(message_output)
        ...
        for batch_idx, batched_data in enumerate(iter_data):
            interaction, scores_1, positive_u, positive_i = eval_func(self.model_1, batched_data)
            interaction, scores_2, positive_u, positive_i = eval_func(self.model_2, batched_data)

            if self.gpu_available and show_progress:
                iter_data.set_postfix_str(set_color('GPU RAM: ' + get_gpu_usage(self.device), 'yellow'))

            # scores_1 and scores_2 are not in the same scalability
            topk_idx, _ = self.eval_collector_fusion.eval_batch_collect(scores_1, scores_2, valid_users_confidence_1, valid_users_confidence_2, interaction, positive_u, positive_i)
        
        self.eval_collector_fusion.model_collect(self.model_1)
        struct = self.eval_collector_fusion.get_data_struct()

        from recbole.evaluator.metrics import NDCGX
        ndcgx = NDCGX(self.config_1)
        ndcg_users = ndcgx.calculate_metric(struct)[:, self.config_1['topk'][0]-1]
