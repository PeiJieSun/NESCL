...
class Dataset_X(object):
    ...
    def __init__(self, config, pre_flag=0, pre_field2id_token=None, pre_field2token_id=None):
        self.pre_flag = pre_flag # 0 denotes default, 1 denotes the research data in other papers

        if self.pre_flag == 1:
            self.pre_field2id_token = pre_field2id_token
            self.pre_field2token_id = pre_field2token_id
    ...
    def _remap_ID_all(self):
        ...
        for alias in self.alias.values():
            ...
            if self.pre_flag == 0:
                ...
                self._remap_x(remap_list, self.pre_field2id_token, self.pre_field2token_id)
    ...
    def _remap_x(self, remap_list, pre_field2id_token, pre_field2token_id):
        """Remap tokens using :meth:`pandas.factorize`.
        ...
        new_ids_list = [1]
        for (feat, field, ftype), _ in zip(remap_list, new_ids_list):
            tokens = feat[field]
            new_ids = []

            try:
                for old_id in tokens:
                    new_ids.append(pre_field2token_id[field][old_id])
            except:
                import pdb; pdb.set_trace()
            feat[field] = new_ids

            self.field2id_token[field] = pre_field2id_token[field]
            self.field2token_id[field] = pre_field2token_id[field]
